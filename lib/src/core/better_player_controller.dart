...
class BetterPlayerController {
  ...
  void setupTranslations(Locale locale) {
    ...
    if (locale != null) {
      ...
      translations = betterPlayerConfiguration.translations?.firstWhereOrNull((translations) => translations.languageCode == languageCode) ?? _getDefaultTranslations(locale);
    } else {
  ...
  Future<void> setupPipController() {
    if (videoPlayerController == null) {
      throw StateError("The data source has not been initialized");
    }
    return videoPlayerController!.setupPipController();
  }
  ...
  Future<void> setFrame(GlobalKey betterPlayerGlobalKey) async {
    final RenderBox? renderBox = betterPlayerGlobalKey.currentContext!.findRenderObject() as RenderBox?;
    if (renderBox == null) {
      BetterPlayerUtils.log("Can't show PiP. RenderBox is null. Did you provide valid global"
          " key?");
    ...
    final Offset position = renderBox.localToGlobal(Offset.zero);
    return videoPlayerController!.setFrame(
      left: position.dx,
      top: position.dy,
      width: renderBox.size.width,
      height: renderBox.size.height,
  ...
  Future<void> updateFrame(GlobalKey betterPlayerGlobalKey) async {
    final RenderBox? renderBox = betterPlayerGlobalKey.currentContext!.findRenderObject() as RenderBox?;
    if (renderBox == null) {
      BetterPlayerUtils.log("Can't show PiP. RenderBox is null. Did you provide valid global"
          " key?");
  ...
  Future<void> removeFrame() async {
    ...
    return videoPlayerController!.removeFrame();
  }

  Future<void> setCanStartPictureInPictureAutomaticallyFromInline(bool canStartPictureInPictureAutomaticallyFromInline) {
    if (videoPlayerController == null) {
      throw StateError("The data source has not been initialized");
    }
    return videoPlayerController!.setCanStartPictureInPictureAutomaticallyFromInline(canStartPictureInPictureAutomaticallyFromInline);
  }
